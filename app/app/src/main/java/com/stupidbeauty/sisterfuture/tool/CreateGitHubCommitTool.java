...
public class CreateGitHubCommitTool implements Tool {
    ...
    private String createNestedTree(OkHttpClient client, String token, String owner, String repo, 
        ...
        // 检查最外层目录是否已存在
        String existingOutermostSha = findExistingDirectory(client, token, owner, repo, baseTreeSha, outermostDirName);
        
        JSONArray finalTreeArray;
        if (existingOutermostSha != null) {
            // 最外层目录已存在，合并内容
            Log.d(TAG, "Outermost directory '" + outermostDirName + "' already exists, merging...");
            finalTreeArray = getTreeEntries(client, token, owner, repo, baseTreeSha);
     ...
     */
    private String findExistingDirectory(OkHttpClient client, String token, String owner, String repo, 
                                         String treeSha, String dirName) throws IOException, org.json.JSONException {
     ...
     * 获取 tree 的所有条目
     */
    private JSONArray getTreeEntries(OkHttpClient client, String token, String owner, String repo, String treeSha) 
            throws IOException, org.json.JSONException {
     ...
     */
    private String mergeWithExistingTree(OkHttpClient client, String token, String owner, String repo, 
                                         String existingTreeSha, String newChildTreeSha, String childName) 
            throws IOException, org.json.JSONException {
        // 获取已存在 tree 的所有条目
