...
public class FtpFileRequestTool implements Tool {
    ...
    private static final int MAX_FILE_SIZE_FOR_CONTENT = 1024 * 1024;
    ...
    public JSONObject getDefinition() {
        try {
            ...
            functionDef.put("description", "从 FTP 服务器读取文件内容。支持文本和二进制文件。");
            ...
            properties.put("save_to_phone", new JSONObject()
                .put("type", "boolean")
                .put("description", "是否将文件保存到手机存储"));
            properties.put("phone_path", new JSONObject()
                .put("type", "string")
                .put("description", "手机保存路径"));
    ...
    private WriteResult writeToPhoneStorage(String path, byte[] content) {
        ...
        try {
            ...
            fos.close();
            Log.d(TAG, "成功写入：" + path);
            return new WriteResult(path, true, "已保存");
    ...
    private WriteResult fallbackToPrivateStorage(String fileName, byte[] content) {
        try {
            ...
            if (privateDir == null) privateDir = context.getFilesDir();
            ...
            return new WriteResult(privatePath, true, "已写入私有目录");
        } catch (IOException e) {
            return new WriteResult("", false, "写入失败：" + e.getMessage());
        }
    }
    ...
    public String getDefaultSystemPromptEnhancement() {
        return "必须在用户明确要求获取 FTP 文件内容时才调用此工具。";
    }
}