...
import com.stupidbeauty.sisterfuture.util.LogUtils;
 ...
 * 
 * @author 太极美术工程狮狮长
 * @version 2.0.2 - 使用 LogUtils 输出日志到文件
 */
public class EstablishTaskRelationshipTool implements Tool {
    ...
    public JSONObject getDefinition() {
        try {
            ...
            logUtils.e(TAG, "Failed to build definition", e);
    ...
    public void executeAsync(@NonNull JSONObject arguments, @NonNull OnResultCallback callback) {
        executor.execute(() -> {
            try {
                ...
                if (blockingIds != null) {
                    for (int i = 0; i < blockingIds.length(); i++) {
                        ...
                        if (blockedId > 0) {
                            ...
                            // 发起 POST 请求
                            logUtils.d(TAG, "🚀 创建关系：" + taskId + " blocks " + blockedId);
                            sendPostRequest(redmineUrl + "/issues/" + taskId + "/relations.json", username, password, requestBody.toString());
                            logUtils.d(TAG, "✅ 关系创建成功：" + blockedId);
                        }
                    }
                }
                ...
                logUtils.i(TAG, "✅ 执行完成：task_id=" + taskId + ", blocked_by_count=" + result.getInt("blocked_by_count") + ", blocking_count=" + result.getInt("blocking_count"));
                callback.onResult(result);

            } catch (Exception e) {
                logUtils.e(TAG, "❌ 执行出错：" + e.getMessage(), e);
    ...
    private void sendPostRequest(String urlString, String username, String password, String body) throws Exception {
        logUtils.d(TAG, "📡 发送 POST 请求到：" + urlString);
        logUtils.d(TAG, "📝 请求体：" + body);
        ...
        logUtils.d(TAG, "📊 响应码：" + responseCode);
