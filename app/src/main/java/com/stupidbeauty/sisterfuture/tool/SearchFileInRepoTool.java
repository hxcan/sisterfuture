            ...
            try {
                ...
                if (results.length() == 0) {
                    String note = "GitHub Code Search API 可能未索引目标仓库，或搜索接口本身不可靠\n" +
                                  ...
                                  "- 使用 `getGitHubFile` 工具直接读取已知路径的文件\n" +
                                  "- 通过 `listFtpDirectory` 或 `listPhoneDirectory` 扫描本地/FTP 目录\n" +
                                  "- 询问用户确认文件是否存在或提供确切路径\n" +
                                  "- 尝试不同的 fileNamePattern 或 pathPattern 组合\n" +
                                  "- 不要重复使用 searchFileInRepo 工具进行相同条件的搜索";
