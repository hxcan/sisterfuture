...
未来姐姐的主项目仓库。这是一个**独立的安卓应用**，作为**纯云端大模型推理的客户端界面**，所有核心AI功能均依赖外部大语言模型接口（如阿里云百炼、Qwen API等）。

## 项目简介
- **核心功能**: 一个高度拟人化的AI助手，通过调用外部LLM API实现复杂的任务管理、情感交互和自我学习能力。
- **运行模式**: **100% 依赖云端服务**，无任何本地AI推理能力。
- **技术栈**: Android, Java/ObjectBox, GitHub Actions.
...
5. 连接设备或启动模拟器进行调试。
6. **网络要求**: 必须配置有效的大模型API密钥才能使用核心AI功能。

**注意**: 此仓库包含敏感信息（如API密钥），请勿公开分享。
